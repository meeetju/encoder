...
class TestEncoder:
    ...
    def test_string_is_cesar_encoded_to_string(self):
        ...
        encoder = Encoder(StringReader('dude lol'), StringWriter(), Cesar(2))
        encoder.encode()
    ...
    def test_file_is_cesar_encoded_to_string(self, file_mock_set):
        ...
        encoder = Encoder(FileReader('dude lol'), StringWriter(), Cesar(2))
        encoder.encode()
        result_string = encoder.get()

        self.open_mock.return_value.read.assert_called_once_with()
        assert result_string == 'fwfg"nqn'

    def test_string_is_cesar_encoded_to_file(self, file_mock_set):

        encoder = Encoder(StringReader('dude lol'), FileWriter('C:\\destination.txt'), Cesar(2))
        encoder.encode()
        encoder.get()
    ...
    def test_string_is_xor_encoded_to_string(self):
        ...
        encoder = Encoder(StringReader('dude lol'), StringWriter(), Xor(3))
        encoder.encode()
        result_string = encoder.get()
